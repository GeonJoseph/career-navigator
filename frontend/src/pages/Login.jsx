...
const Login = () => {
    ...
    React.useEffect(() => {
        if (location.state?.authError) {
            ...
            if (err.includes('missing')) setError("Social login is currently unavailable. Please use email or create an account.");
            else if (err === 'auth_failed') setError("Authentication failed. Please try again.");
            else if (err === 'no_email') setError("No email address found from the provider.");
            else setError("An error occurred during social login.");
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            if (data.message === "2FA Required" || data.message === "Verification Required") {
            ...
            <div className="min-h-screen flex flex-col items-center pt-20 p-4">
                <div className="flex flex-col items-center mb-8">
                    ...
                    <p className="text-slate-400 text-center max-w-sm">
                        Please enter the 6-digit verification code we just sent to <span className="text-white font-semibold">{email}</span>.
                    </p>
                </div>
                ...
                <div className="p-8">
                    ...
                    <form onSubmit={handleLogin} className="space-y-5">
                        ...
                        <div className="flex items-center justify-between pt-1">
                            <label className="flex items-center gap-2 cursor-pointer">
                                <input 
                                    ...
                                    className="w-4 h-4 rounded border-white/10 text-blue-600 focus:ring-blue-500 bg-slate-900/50" 
                                />
                        ...
                        <button
                            type="submit"
                            disabled={isLoading}
                            className={`w-full py-3 bg-blue-600 text-white rounded-lg font-bold transition-colors shadow-lg shadow-blue-900/30 flex items-center justify-center gap-2 ${
                                isLoading ? "opacity-75 cursor-not-allowed" : "hover:bg-blue-500 active:scale-95"
                            }`}
                        >
                        ...
                        <div className="relative mt-6 mb-6">
                            ...
                            <div className="relative flex justify-center text-sm">
                                <span className="px-4 bg-transparent text-slate-500 backdrop-blur-md" style={{ backgroundColor: 'rgba(15, 23, 42, 0.4)' }}>Or continue with</span>
                            </div>
                        </div>
